...
export async function POST(_: Request, { params }: { params: Promise<{ id:string }> }) {
  ...
  const { data:battle } = await supabase.from('coding_battles').select('id,status,batch_id,max_participants').eq('id', id).single();
  if (!battle || !['SCHEDULED','LIVE','LOBBY'].includes(battle.status) || (battle.batch_id && battle.batch_id !== profile?.graduation_period)) return NextResponse.json({ error:'You are not eligible to join this battle.' }, { status:403 });
  ...
  if ((count || 0) >= maxLimit) return NextResponse.json({ error:`This battle has reached its ${maxLimit} participant limit.` }, { status:409 });

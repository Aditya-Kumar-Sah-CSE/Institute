...
export default function TTTInviteJoinPage() {
  ...
  const handleJoin = async () => {
    try {
      ...
      if (data.success || data.isHost) {
        router.push(`/games/tic-tac-toe/${gameId}`);
      } else {
  ...
  if (errorMsg || !session) {
    return (
      <div className="games-lobby-container" style={{ textAlign: 'center', padding: 60, maxWidth: 500 }}>
        ...
        <Link href="/games" style={{ background: 'linear-gradient(135deg, #06b6d4, #ec4899)', color: '#fff', padding: '10px 20px', borderRadius: 10, fontSize: '13px', fontWeight: 800, textDecoration: 'none' }}>
          Explore Games Lobby
        </Link>
      </div>
    );
  ...
  return (
    <div className="games-lobby-container" style={{ maxWidth: 520 }}>
      <div style={{ background: 'rgba(15, 23, 42, 0.8)', border: '1px solid rgba(6, 182, 212, 0.3)', borderRadius: 24, padding: 32, textAlign: 'center', backdropFilter: 'blur(16px)' }}>
        <span style={{ fontSize: '42px', display: 'block', marginBottom: 12 }}>❌ ⭕</span>
        
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(6,182,212,0.15)', border: '1px solid rgba(6,182,212,0.3)', color: '#06b6d4', padding: '4px 12px', borderRadius: 20, fontSize: '11px', fontWeight: 800, marginBottom: 16 }}>
        ...
        <p style={{ fontSize: '13px', color: '#94a3b8', margin: '0 0 24px 0' }}>
          Tic-Tac-Toe 3x3 Neon Grid Arena
        </p>
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '14px 18px', marginBottom: 24, textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            ...
            <span style={{ fontWeight: 800, color: '#fff' }}>{hostName} (X)</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span style={{ color: '#64748b' }}>Your Symbol</span>
            <span style={{ fontWeight: 800, color: '#ec4899' }}>Guest (O)</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span style={{ color: '#64748b' }}>Game Type</span>
            <span style={{ fontWeight: 800, color: '#06b6d4' }}>Tic-Tac-Toe</span>
          </div>
        </div>
        ...
        <button
          onClick={handleJoin}
          disabled={joining}
          style={{ width: '100%', padding: '14px', borderRadius: 12, background: 'linear-gradient(135deg, #06b6d4, #ec4899)', border: 'none', color: '#fff', fontSize: '14px', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 0 20px rgba(6,182,212,0.3)' }}
        >

...
export default function AISettingsClient() {
  ...
  const handleSaveKey = async (provider: AIProviderName) => {
    const rawKey = provider === 'gemini' ? geminiKeyInput : grokKeyInput;
    if (!rawKey.trim()) {
  ...
  const handleTestConnection = async (provider: AIProviderName) => {
    ...
    const typedKey = provider === 'gemini' ? geminiKeyInput.trim() : grokKeyInput.trim();
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: 'var(--space-md)' }}>
      {/* BREADCRUMB HEADER */}
      <div style={{ marginBottom: 'var(--space-md)' }}>
        ...
        <p style={{ margin: '6px 0 0 0', fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>
          Connect your personal Google Gemini or xAI Grok API key. Keys are encrypted at rest on the server and never exposed to client scripts.
        </p>
      </div>
      ...
      <div style={{ 
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <HelpCircle size={18} /> How to connect?
          </span>
          {isGuideOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
        </button>

        {isGuideOpen && (
          <div style={{ padding: '0 18px 18px 18px', borderTop: '1px solid var(--glass-border)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px', paddingTop: '16px' }}>
            {/* GEMINI GUIDE */}
            <div style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '14px', borderRadius: '10px', border: '1px solid var(--glass-border)' }}>
              ...
              <ol style={{ margin: 0, paddingLeft: '18px', fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                ...
                <li>Paste it into Smart Learn.</li>
            ...
            <div style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '14px', borderRadius: '10px', border: '1px solid var(--glass-border)' }}>
              ...
              <ol style={{ margin: 0, paddingLeft: '18px', fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                ...
                <li>Create/sign in to your account.</li>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '16px', paddingTop: '14px', borderTop: '1px solid var(--glass-border)' }}>
            ...
            {geminiInfo && (
              <>
                ...
                >
                  {isTestingGemini ? <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> : 'Test Connection'}
                </Button>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '16px', paddingTop: '14px', borderTop: '1px solid var(--glass-border)' }}>
            ...
            {grokInfo && (
              <>
                ...
                >
                  {isTestingGrok ? <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> : 'Test Connection'}
                </Button>

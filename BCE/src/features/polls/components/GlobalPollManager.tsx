...
import { createGlobalPoll } from '../actions';
import { Send, Plus, X, BarChart2 } from 'lucide-react';

interface GlobalPollManagerProps {
  hideHeading?: boolean;
}

export default function GlobalPollManager({ hideHeading = false }: GlobalPollManagerProps) {
  ...
  const handleCreatePoll = async () => {
    ...
    try {
      const result = await createGlobalPoll(question.trim(), validOptions, isMultipleChoice, expiresInDays);
      if (result.error) {
        setError(result.error);
      } else {
        setSuccess('Global poll created successfully.');
        setQuestion('');
        setOptions(['', '']);
  ...
  return (
    <>
      <div 
        className={`create-global-poll-header ${hideHeading ? 'create-poll-widget-header-compact' : ''}`} 
        style={{ 
          display: 'flex', 
          justifyContent: hideHeading ? 'flex-end' : 'space-between', 
          alignItems: 'center', 
          marginBottom: hideHeading ? 0 : 'var(--space-lg)' 
        }}
      >
        {!hideHeading && <h2 className="section-title" style={{ margin: 0 }}>Global Polls</h2>}
        {!isExpanded && (
          <button 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
            <div className="create-poll-form-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
              <h3 style={{ color: 'var(--neon-cyan)', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                <BarChart2 size={20} /> Create New Global Poll
              </h3>
              
              <div className="create-poll-controls" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', gap: '0.25rem', background: 'var(--bg-body)', padding: '0.25rem', borderRadius: 'var(--radius-md)', border: '1px solid rgba(0, 240, 255, 0.2)' }}>
                  <button 
                    type="button"
                    ...
                    style={{ 
                      ...
                      border: 'none', 
                      background: isMultipleChoice ? 'var(--neon-cyan)' : 'transparent',
                      color: isMultipleChoice ? '#000' : 'var(--text-muted)',
                      fontWeight: 'bold',
            ...
            {error && <div style={{ color: 'var(--neon-pink)', fontSize: 'var(--text-sm)' }}>{error}</div>}
            {success && <div style={{ color: 'var(--neon-lime)', fontSize: 'var(--text-sm)' }}>{success}</div>}
            ...
            <div className="create-poll-options" style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '0 8px' }}>
                ...
                <div key={idx} className="create-poll-option-row" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {options.length > 2 && (
                    <button 
                      ...
                      disabled={isSubmitting}
                      style={{ background: 'transparent', border: 'none', color: 'var(--neon-pink)', cursor: 'pointer', padding: '0.25rem' }}
                    >
                      <X size={16} />
                    </button>
                  )}
              ...
              {options.length < 10 && (
                <button 
                  ...
                  disabled={isSubmitting}
                  style={{ alignSelf: 'flex-start', background: 'transparent', border: 'none', color: 'var(--neon-cyan)', display: 'flex', alignItems: 'center', gap: '0.25rem', cursor: 'pointer', fontSize: '0.85rem', padding: '0.25rem 0.5rem', marginTop: '0.25rem' }}
                >
            ...
            <div className="create-poll-question-row" style={{
              ...
              <input 
                placeholder="Ask your global poll question here..."
                value={question}
              ...
              <button 
                ...
                style={{
                  background: (question.trim() && options.filter(o => o.trim()).length >= 2) ? 'var(--neon-cyan)' : 'rgba(255, 255, 255, 0.1)',
                  color: (question.trim() && options.filter(o => o.trim()).length >= 2) ? '#000' : 'rgba(255, 255, 255, 0.3)',
                  border: 'none',
                  borderRadius: '50%',
                  width: '2.5rem',
                  height: '2.5rem',

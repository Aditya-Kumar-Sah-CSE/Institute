...
export default function ChatComposer({ msgInput, setMsgInput, handleSend, isSomeoneTyping }: ChatComposerProps) {
  return (
    <div style={{ padding: 'var(--space-md)', background: 'var(--bg-secondary)', borderTop: '1px solid var(--border-divider)', position: 'relative' }}>
        {isSomeoneTyping && (
          <div 
            style={{ 
              position: 'absolute', 
        ...
        <form onSubmit={handleSend} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
          ...
          <button 
            ...
            style={{ 
              ...
              boxShadow: msgInput.trim() ? '0 4px 10px rgba(0, 240, 255, 0.3)' : 'none'
            }}
          >

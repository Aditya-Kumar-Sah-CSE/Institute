...
async function main() {
  const { data: profile } = await supabase.from('profiles').select('*').eq('email', 'adityakumarsah@gmail.com').maybeSingle();
  console.log('Profile:', profile);
  
  const { data: inst } = await supabase.from('institutions').select('*').like('slug', '%bce-bhagalpur%').maybeSingle();
  console.log('Institution:', inst);

  if (profile && inst) {
    console.log('Match?', profile.institution_id === inst.id);
  }
}
main();

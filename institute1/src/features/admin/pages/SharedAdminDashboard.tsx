...
import { BookOpen, FileText, Users, Table, Pin, PinOff, UserPlus } from 'lucide-react';
import ExpandableSettingsCard from '@/features/admin/components/ExpandableSettingsCard';
import { SUPER_ADMIN_EMAIL } from '@/lib/constants';
import { RequestContext } from '@/lib/context/requestContext';

export default async function SharedAdminDashboard({ context }: { context: RequestContext }) {
  ...
  async function updateSettings(formData: FormData) {
    ...
    if (currentSettings) {
      ...
      await sb.from('company_settings').update({ company_name: name, tagline: tagline, logo_url: finalLogoUrl }).eq('id', currentSettings.id);
  ...
  async function updateInstSettings(formData: FormData) {
    ...
    await adminSb.from('institutions').update({ name: name, logo: finalLogoUrl }).eq('id', p.institution_id);
    revalidatePath('/admin');
  }
  ...
  async function toggleAdmissionPin() {
    ...
    const { data: currentSettings } = await sb.from('company_settings').select('*').limit(1).single();
    if (currentSettings) {
      await sb.from('company_settings').update({ is_admission_pinned: !currentSettings.is_admission_pinned }).eq('id', currentSettings.id);
      revalidatePath('/admin');
      ...
      <div className="dashboard-stats-grid">
        <Link href="/admin/students" style={{ textDecoration: 'none' }}>
          <Card variant="glass" padding="lg" className="stat-card hover-lift" style={{ height: '100%' }}>
            ...
            <div className="stat-card-content">
              <div className="stat-card-value" style={{ color: 'var(--accent-primary)' }}>{studentCount || 0}</div>
        ...
        <Link href="/admin/courses" style={{ textDecoration: 'none' }}>
          <Card variant="glass" padding="lg" className="stat-card hover-lift" style={{ height: '100%' }}>
            ...
            <div className="stat-card-content">
              <div className="stat-card-value" style={{ color: 'var(--accent-secondary)' }}>{courseCount || 0}</div>
        ...
        <Link href="/admin/submissions" style={{ textDecoration: 'none' }}>
          <Card variant="glass" padding="lg" className="stat-card hover-lift" style={{ height: '100%' }}>
            ...
            <div className="stat-card-content">
              <div className="stat-card-value" style={{ color: 'var(--accent-warning)' }}>{submissionCount || 0}</div>
        ...
        <Link href="/admin/instructor-requests" style={{ textDecoration: 'none' }}>
          <Card variant="glass" padding="lg" className="stat-card hover-lift" style={{ height: '100%' }}>
            ...
            <div className="stat-card-content">
              <div className="stat-card-value" style={{ color: 'var(--accent-success)' }}>{instructorRequestCount || 0}</div>
        ...
        {isSuperAdmin ? (
          <ExpandableSettingsCard title="Global Platform Settings">
            <form action={updateSettings} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
              <Input name="company_name" label="Global Platform Name" defaultValue={settings?.company_name || ''} />
              <Input name="tagline" label="Global Tagline" defaultValue={settings?.tagline || ''} />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xs)' }}>
                <label style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>Global Logo</label>
                <input type="file" name="logo_file" accept="image/*" style={{ padding: 'var(--space-sm)', background: 'var(--bg-input)', color: 'var(--text-primary)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-default)', maxWidth: '100%', boxSizing: 'border-box' }} />
                  {settings?.logo_url && (
              ...
              <div style={{ marginTop: 'var(--space-sm)' }}><Button type="submit" variant="primary">Save Global Settings</Button></div>
            </form>
          ...
          <ExpandableSettingsCard title="Institute Settings">
            <form action={updateInstSettings} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
              <Input name="company_name" label="Institute Name" defaultValue={instSettings?.name || ''} />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xs)' }}>
              ...
              <div style={{ marginTop: 'var(--space-sm)' }}><Button type="submit" variant="primary">Save Settings</Button></div>
            </form>
          </ExpandableSettingsCard>
        )}

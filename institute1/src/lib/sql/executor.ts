...
export class SQLExecutor {
  ...
  constructor(db: Database) {
    // Deep clone initial database state for execution workspace
  ...
  private executeSelect(ast: ASTSelectStatement): { columns: ColumnMetadata[]; rows: Row[] } {
    ...
    // 1. Determine base source dataset
    let sourceRows: Row[] = [{}];
    let availableTables: Record<string, string> = {}; // alias -> actual table name
    ...
    // 4. Process GROUP BY & Aggregations
    ...
    if (ast.columns.length === 1 && ast.columns[0].expr.type === 'STAR') {
      ...
      if (sourceRows.length > 0) {
        ...
        const keysToInclude = allKeys.filter(k => {
          if (k.includes('.')) {
            ...
            return true; // Keep prefixed keys first
          }
          return false;
        });
      ...
      projectedRows = sourceRows.map(row => {
        ...
        for (let i = 0; i < ast.columns.length; i++) {
          ...
          // Check if already computed in aggregate step
  ...
  private executeInsert(ast: ASTInsertStatement): number {
    ...
    for (const tuple of ast.values) {
      ...
      // Check primary key auto generation
  ...
  private evaluateExpression(expr: ASTExpression, row: Row): any {
    ...
    switch (expr.type) {
      ...
      case 'IDENTIFIER': {
        ...
        // Search case insensitive or prefixed keys
  ...
  // Scalar SQL Functions (UPPER, LOWER, LENGTH, CONCAT, ROUND, ABS, CEIL, FLOOR)

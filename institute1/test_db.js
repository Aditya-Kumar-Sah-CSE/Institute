...
async function check() {
  const { data: courses } = await supabase
    .from('courses')
    .select('title, institution_id, profiles(name, institution_id)');
  require('fs').writeFileSync('courses_dump_2.json', JSON.stringify(courses, null, 2));
  console.log("Wrote to courses_dump_2.json");
}

...
**Identified Missing FK Indexes:**
- `reply_votes.user_id`
- `user_badges.badge_id`
- `badges.course_id`
- `doubts.user_id`
- `doubt_replies.user_id`
- `doubt_replies.doubt_id`
- `doubts.course_id`
- `doubts.lesson_id`
- `doubt_replies.parent_id`
- `doubt_views.user_id`
- `notifications.user_id`

*Impact: High. Adding these will drastically speed up JOINs and CASCADE operations.*

### RLS Optimization (Row Level Security)
Some RLS policies use subqueries (`EXISTS (SELECT ...)`). At scale, if the joined tables lack indexes, these policies will cause sequential scans for *every* row evaluated.

**Policies to Watch:**
- `instructor_applications`: Admins can view all instructor applications
- `courses`: Instructors can update their own courses
- `enrollments`: Instructors view enrollments for their courses
- `enrollments`: Instructors update enrollments for their courses
- `feedbacks`: Admins and instructors can view all feedbacks

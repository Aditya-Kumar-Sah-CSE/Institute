              ...
              <div className="mobile-dropdown">
                 ...
                 {(currentView as string) === 'admin' && (
                   <>
                     ...
                     <button 
                       type="button" 
                       className="navbar-dropdown-item text-red"
                       style={{ marginTop: '0.5rem', width: '100%', textAlign: 'left', padding: '12px 16px', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--text-md)', display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}
                       onClick={() => signOut()}
                     >
                 ...
                 {(currentView as string) === 'instructor' && (
                   <>
                 ...
                 {(currentView as string) !== 'student' && (
                   <Link href="/dashboard" onClick={() => setIsMenuOpen(false)}>
                     {getIcon('Dashboard', { size: 16, className: 'mobile-nav-icon' })} Student View
                   </Link>
                 )}
                 {(currentView as string) !== 'admin' && profile.role === 'admin' && (
                   <Link href="/admin" onClick={() => setIsMenuOpen(false)}>
                     {getIcon('Admin', { size: 16, className: 'mobile-nav-icon' })} Admin Panel
                   </Link>
                 )}
                 {(currentView as string) !== 'instructor' && ((profile.role === 'instructor' && profile.status === 'active') || profile.role === 'admin') && (
                 ...
                 <Link href="/profile" onClick={() => setIsMenuOpen(false)}>
                  ...
                  <button type="button" className="mobile-logout-btn" onClick={() => signOut()}>
                     {getIcon('Logout', { size: 16, className: 'mobile-nav-icon' })} Logout
                   </button>
              </div>
            )}
